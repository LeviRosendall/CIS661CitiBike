...
for (station in Station_names){
  rm("CleanContPlotting", "newContPlotting", "newCleanOnePlotting", "cleanOnePlotTable","cleanOnePlotTableLastDay")
  CleanContPlotting <- Plotting %>% 
    mutate(contaminated=ifelse(starting==1 & station_name=="1 Ave & E 16 St", 1, 0)) %>% 
    select(bikeid, contaminated)
